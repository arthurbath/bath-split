ALTER TABLE public.budget_expenses
  ADD COLUMN IF NOT EXISTS current_period_handling text NOT NULL DEFAULT 'include_current_period';

ALTER TABLE public.budget_income_streams
  ADD COLUMN IF NOT EXISTS current_period_handling text NOT NULL DEFAULT 'include_current_period';

UPDATE public.budget_expenses
SET current_period_handling = 'include_current_period'
WHERE current_period_handling NOT IN ('include_current_period', 'exclude_current_period_until_closed');
...
ALTER TABLE public.budget_expenses
  DROP CONSTRAINT IF EXISTS budget_expenses_current_period_handling_check;

ALTER TABLE public.budget_expenses
  ADD CONSTRAINT budget_expenses_current_period_handling_check
    CHECK (current_period_handling IN ('include_current_period', 'exclude_current_period_until_closed'));

ALTER TABLE public.budget_income_streams
  DROP CONSTRAINT IF EXISTS budget_income_streams_current_period_handling_check;

ALTER TABLE public.budget_income_streams
  ADD CONSTRAINT budget_income_streams_current_period_handling_check
    CHECK (current_period_handling IN ('include_current_period', 'exclude_current_period_until_closed'));
...
BEGIN
  ...
  IF jsonb_array_length(v_incomes) > 0 THEN
    ...
    SELECT
      ...
      END,
      CASE
        WHEN x.current_period_handling IN ('include_current_period', 'exclude_current_period_until_closed')
          THEN x.current_period_handling
        ELSE 'include_current_period'
      END,
      CASE
